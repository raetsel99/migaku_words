...
if __name__ == '__main__':
    ...
    if args.json_input:
        agnostic_jason_in = Path(args.json_input)
        if os.path.exists(agnostic_jason_in):
            migaku_array = read_json_in(agnostic_jason_in)
            print(f"json is {migaku_array}")
        else:
            print(f"ERROR E001 could not find file {agnostic_jason_in}")
